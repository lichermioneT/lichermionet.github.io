                ...
                {   
                  ...
                  // 右边树最小的位置。
                    ...
                    */
                    ...
                    {
                        rightMinParent->right = rightMin->right;// 循环没有进去，直接就是
                    }
                    delete rightMin;
                }
...
```
3 4 6 7 22 24 33 47 55 56 69 89 
3 4 6 7 24 33 47 55 56 69 89 
3 4 6 7 24 33 47 56 69 89 
3 4 7 24 33 47 56 69 89 
3 4 24 33 47 56 69 89 
3 24 33 47 56 69 89 
24 33 47 56 69 89 
24 33 47 56 89 
33 47 56 89 
...
struct BSTNode
{
    explicit BSTNode(const K& key)   // explicit 禁止隐士类型转换的
        : _left(nullptr)
        , _right(nullptr)
        , _key(key)                 // 需要存入的数据
    {}
...
Node* Find(const K& key)
{
    ...
    while (cur != nullptr)
    {
        if (key < cur->_key)      // 查找值小于当前值就在：左边 
        {
            cur = cur->_left;
        }
        else if (key > cur->_key) // 查找值大于当前值就在：右边 
        {
            cur = cur->_right;
        }
        else                      // 等于
...
private:
    struct Node
    {
        explicit Node(const K& key)    // 禁止隐士类型转换
            : _left(nullptr)
            , _right(nullptr)
            , _key(key)
        {}
...
public:
    ...
    Node* Find(const K& key)
    {
        ...
        while (cur != nullptr)
        {
            if (key < cur->_key)          // 小于当前节点，左边子节点去
            {
                cur = cur->_left;
            }
            else if (key > cur->_key)    // 大于当前节点，右边子节点去
            {
                cur = cur->_right;  
            }
            else                         // 等于当前节点
